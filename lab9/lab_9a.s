...
i:				# loop counter
	.long		1
x:				# function argument
	.double		0.0       #albo .float - 4 bajty 
y:
	.double		0.0       #double - 8 bajtów 
...
main:
	FINIT			# FPU initialization          #Jak zaczynamy od litery F  to odnosi się do  koprocesora matematycznego
next:
	FILDL	i		# i -> ST(0)                  #załaduj i na stos. Po wczytaniu liczby całkowitej do koprocesora jest ona autoamtycznie konwertowana do postaci zmiennoprzecinkowej,
	FSTPL	x		# ST(0) -> x & pop from stack #L - suffix od double   Rejestry zorganizowane są jak stosy. Jest 8 rejestrów. Po inicjalizacji stosy są puste. 
				#------------------------------
	...
	FSTPL	y        #Lepszy mov - można z pamięci do pamięci
	CALL	disp      #Obudowane wywołanie printf
...
disp:
	movq	x, %xmm0          #do przenoszenia 8  bajtów 
	movq	y, %xmm1
	mov	$fmt_str, %rdi	# address of fmt_str
	mov $2, %al                #wartość 2 - bo używamy dwóch rejestrów zmiennoprzecinkowych
	CALL	printf
	RET

#Rejestry w SSE - xmm0 - xmm7, każdy ma wielkość 16bajtów - na dwa double.
